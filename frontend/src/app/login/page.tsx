...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const url = new URL(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/auth/token`);
      const res = await fetch(url.toString(), {
        ...
        body: JSON.stringify({
          username: 'user@example.com', // dummy
        ...
        <div className="bg-surface-container-low p-panel-padding rounded border border-outline-variant shadow-xl">
          <form className="space-y-stack-md" onSubmit={handleLogin}>
            <div className="space-y-unit">
              ...
              <div className="relative">
                ...
                <input required type="email" id="email" className="w-full bg-surface-container-lowest border border-outline-variant rounded p-3 pl-10 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/20 input-focus-ring transition-all duration-200" placeholder="name@studio.com" />
              </div>
            </div>
            ...
            <div className="space-y-unit">
              ...
              <div className="relative">
                ...
                <input required type="password" id="password" className="w-full bg-surface-container-lowest border border-outline-variant rounded p-3 pl-10 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/20 input-focus-ring transition-all duration-200" placeholder="••••••••" />
              </div>
            </div>
          ...
          <div className="mt-stack-lg pt-stack-md border-t border-outline-variant/30 flex flex-col gap-stack-sm items-center">
            ...
            <div className="flex items-center gap-unit">
              ...
              <a href="#" className="font-label-md text-label-md text-on-surface-variant hover:text-primary-container underline underline-offset-4 decoration-primary-container/30">Request Access</a>
            </div>
          </div>
        </div>
